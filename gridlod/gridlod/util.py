...
def linearpIndexBasis(N):  # Di: N = Extend of any world, i.e. NCoarse, NFine, Nepsilon etc. 
    """Compute basis b to convert from d-dimensional indices to linear indices.

    Example for d=3:
    
    b = linearpIndexBasis(NWorld)
    ind = np.dot(b, [1,2,3])                #Di: np.dot() ---> matrix multiplication/ dot product

    ind contains the linear index for point (1,2,3).
    """
    cp = np.cumprod(N+1, dtype='int64')  # Di: Return the cumulativ product in the same structure of N in integers
    b = np.hstack([[1], cp[:-1]])
    return b

def convertpLinearIndexToCoordIndex(N, ind):  #Di: N= Any World, ind = linear index set of all points under consideration
    ind = np.array(ind)
    d = np.size(N)
    if ind.ndim > 0:   #Di: If there exists more than one element in the linear index set
        m = np.size(ind)
...
def tCoordinates(NWorld, iPatch=None, NPatch=None):  # Di: Givs the coordinates of the center point of each element as a linear array on any "World"
    if NPatch is None:
        NPatch = NWorld-1
    else:
        NPatch = NPatch-1
...
def pCoordinates(NWorld, iPatch=None, NPatch=None): #Di: Gives the coordinates of nodes of the hypercubes as a linear array on any "World", iPatch ?????
    if iPatch is None:
