...
def compute_combined_MsStiffness(world,Nepsilon,aPert,aRefList, KmsijList,muTPrimeList,k,model,s, compute_indicator=False,
    ...
    def compute_combinedT(TInd):
        ...
        if model['name'] == 'check':
            ...
            if dim == 2:
                ...
                alphaT[:len(alphaT)-1] = (rPatch()[indx]-(alpha*s))/(beta-alpha)                                                
            elif dim == 1:
                alphaT[:len(alphaT)-1] = (rPatch()[np.arange(len(aRefList)-1)*np.prod(NFineperEpsilon)]-(alpha*s))/(beta-alpha) 
            alphaT[len(alphaT)-1] = s-np.sum(alphaT[:len(alphaT)-1])
        elif model['name'] == 'incl':
            alphaT = np.zeros(len(aRefList))
            bgval = model['bgval']
            ...
            alphaT[:len(alphaT)-1] = (inclval - rPatch()[indx]-(alpha*s))/(inclval-bgval)
            alphaT[len(alphaT)-1] = s - np.sum(alphaT[:len(alphaT)-1])
        elif model['name'] == 'inclvalue':
            alphaT = np.zeros(len(aRefList))
            defval = model['defval']
            ...
            indx = (tmp_indx[0] * NFineperEpsilon[1] * patchT[TInd].NPatchFine[0]).astype(int) \
                   + (tmp_indx[1] * NFineperEpsilon[0]).astype(int)
            alphaT[:len(alphaT) - 1] = (inclval - rPatch()[indx](alpha*s)) / (inclval - defval)
            alphaT[len(alphaT) - 1] = s - np.sum(alphaT[:len(alphaT) - 1])
            ...
            indx = (tmp_indx[0] * NFineperEpsilon[1] * patchT[TInd].NPatchFine[0]).astype(int) \
                    + (tmp_indx[1] * NFineperEpsilon[0]).astype(int)
            alphaT[:len(alphaT)-1] = (bgval - rPatch()[indx](alpha*s))/(bgval-inclval)
            alphaT[len(alphaT)-1] = s - np.sum(alphaT[:len(alphaT)-1])
